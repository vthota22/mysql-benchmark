...
FAILOVER_FIELDS: tuple[FieldSpec, ...] = (
    ...
    FieldSpec(
        ...
        "number",
        "Soft cooldown between iterations; GR readiness gate is what waits for full recovery",
        section="Run matrix",
        default="120",
    ),
    ...
    FieldSpec(
        ...
        "Trigger method",
        "select",
        options=("pod_delete", "mysqld_kill", "mysqld_freeze", "set_as_primary"),
        section="Trigger",
    ),
    ...
    FieldSpec(
        "FAILOVER_MYSQLD_FREEZE_SEC",
        "mysqld_freeze hold (sec)",
        "number",
        "Hold primary mysql cgroup frozen (≥ detection + member_expel_timeout)",
        section="Trigger",
        default="25",
    ),
    FieldSpec("FAILOVER_TRIGGER_PREPARE_SEC", "Prepare kubeconfig before trigger (s)", "number", section="Trigger"),
    FieldSpec(
        "FAILOVER_GR_EXPECTED_MEMBERS",
        "GR expected members",
        "number",
        "Require this many ONLINE GR members + Ready mysql pods before trigger (0 = no count check)",
        section="Trigger",
        default="3",
    ),
    FieldSpec(
        "FAILOVER_GR_REQUIRE_K8S_PODS_READY",
        "Require mysql pods Ready (Advanced)",
        "checkbox",
        "Also require all mysql-* pods Ready (not CrashLoop / deleting) before trigger",
        section="Trigger",
        default="1",
    ),
    FieldSpec(
        "FAILOVER_GR_READINESS_TIMEOUT_SEC",
        "GR readiness gate timeout (s)",
        "number",
        "Max wait for full cluster health before aborting the iteration",
        section="Trigger",
        default="600",
    ),
    FieldSpec(
