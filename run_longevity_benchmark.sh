...
_longevity_preserve_env() {
  local _vars=(
    ...
    LONGEVITY_AUTO_RESTART LONGEVITY_EDITIONS LONGEVITY_RATE
  )
  local _v
  for _v in "${_vars[@]}"; do
...
echo "Load:     threads=${LONGEVITY_THREADS:-32} warmup=${LONGEVITY_WARMUP_SEC:-300}s rate=${LONGEVITY_RATE:-0} (0=unthrottled)"
...
run_longevity_edition() {
  ...
  local segment_idx=0
  local consec_fast_fail=0
  local max_fast_fail="${LONGEVITY_MAX_FAST_FAILS:-8}"
  local fast_fail_sec="${LONGEVITY_FAST_FAIL_SEC:-30}"
  local bench_start
  bench_start=$(date +%s)
  ...
  while [[ "${remaining}" -gt 0 ]]; do
    ...
    # Count productive time toward target ONLY for successful segments — a failed
    # or fast-dying segment must not inflate elapsed and mask a restart storm.
    local productive=0
    if [[ "${exit_code}" -eq 0 ]]; then
      productive=$((seg_wall - warmup))
      [[ "${productive}" -lt 0 ]] && productive=0
    ...
    if [[ "${exit_code}" -eq 0 ]]; then
      ...
      # Circuit breaker: segments that die quickly are "fast failures". Too many
      # in a row means the cluster/dataset is unhealthy, so abort instead of
      # spinning forever (an uncapped loop previously spawned thousands of
      # orphaned restart loops and took down the droplet).
      if [[ "${seg_wall}" -lt "${fast_fail_sec}" ]]; then
        consec_fast_fail=$((consec_fast_fail + 1))
      else
        consec_fast_fail=0
      fi
      if [[ "${consec_fast_fail}" -ge "${max_fast_fail}" ]]; then
        echo "FATAL: ${consec_fast_fail} consecutive fast failures (<${fast_fail_sec}s each)."
        echo "       Aborting to avoid a restart storm. Check cluster health (GR/operator)"
        echo "       and the TPC-C dataset, then rerun."
        write_longevity_state "${state_file}" "${total_elapsed}" "${restart_count}" "failed"
        return 1
      fi
      restart_count=$((restart_count + 1))
      local backoff=$((consec_fast_fail * 10))
      [[ "${backoff}" -gt 60 ]] && backoff=60
      echo "WARNING: sysbench exited early (code ${exit_code}, wall=${seg_wall}s); consecutive fast-fails=${consec_fast_fail}/${max_fast_fail}; restart #${restart_count} in ${backoff}s..."
      sleep "${backoff}"
    fi
