...
# Installs to <repo>/sysbench-1.1/ — does not replace system sysbench
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
REPO_ROOT="$(cd "${SCRIPT_DIR}/.." && pwd)"
PREFIX="${SYSBENCH_PREFIX:-${REPO_ROOT}/sysbench-1.1}"
SRC_DIR="${SYSBENCH_SRC:-${REPO_ROOT}/src/sysbench}"
...
if is_macos; then
  ...
  for cmd in git gcc make autoconf automake libtoolize pkg-config mysql_config; do
    if ! command -v "$cmd" >/dev/null 2>&1; then
      echo "ERROR: Missing '${cmd}'. Run bootstrap/setup_benchmark.sh or:"
