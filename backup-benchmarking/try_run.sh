...
# No backup profiling, no database init — just connect, run, and print results.
#
# Usage:
...
# Only need DB connection vars, not backup profiling config.
...
# Disable backup profiling for try-run preflight
export SKIP_BACKUP_PROFILING=1
