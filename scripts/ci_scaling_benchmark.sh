#!/usr/bin/env bash
# Orchestrate a scaling benchmark on a remote droplet (GitHub Actions scheduler or local).
#
...
DROPLET_SSH_PORT=22
REMOTE_REPO=""
REMOTE_BENCHMARK_CONF="scaling-benchmarking/benchmark.conf"
CI_GIT_SYNC=1
DROPLET_GIT_BRANCH=""
...
_dump_failure_diagnostics() {
  ...
  ctl_q="$(printf '%q' "${REMOTE_REPO}/scripts/scaling_run_ctl.sh")"

  {
    ...
    _ssh "tail -n 100 ${repo_q}/scaling-benchmarking/results/control_wrapper.log" 2>&1 || true
    if [[ -n "${RESULTS_DIR:-}" ]]; then
      echo
      echo "----- remote: ${RESULTS_DIR}/benchmark.log (tail 100) -----"
      _ssh "tail -n 100 ${repo_q}/${RESULTS_DIR}/benchmark.log" 2>&1 || true
    fi
...
_preflight_remote() {
  ...
  conf_q="$(printf '%q' "$(_remote_conf_path)")"
  ctl_q="$(printf '%q' "${REMOTE_REPO}/scripts/scaling_run_ctl.sh")"
  run_q="$(printf '%q' "${REMOTE_REPO}/scaling-benchmarking/run_benchmark.sh")"
...
_ctl_status() {
  ...
  conf_q="$(printf '%q' "$(_remote_conf_path)")"
  ctl_q="$(printf '%q' "${REMOTE_REPO}/scripts/scaling_run_ctl.sh")"
  _ssh_capture "scaling_run_ctl status" \
...
_start_run() {
  ...
  _step "Starting scaling benchmark on droplet"
  start_out="$(_ssh_capture "scaling_run_ctl start" "set -euo pipefail; cd ${repo_q}; BENCHMARK_CONF=${conf_q} ${ctl_q} start")"
  echo "${start_out}"
  ...
  if [[ -z "${results_dir}" || "${results_dir}" == "scaling-benchmarking/results/run_pending" ]]; then
...
_poll_until_complete() {
  ...
  while [[ "${elapsed}" -lt "${CI_MAX_WAIT_SEC}" ]]; do
    ...
    if [[ "${RUNNING:-0}" != "1" && -n "${RESULTS_DIR:-}" ]]; then
      ...
      if _ssh "grep -q 'PHASE=DONE' $(printf '%q' "${REMOTE_REPO}/${RESULTS_DIR}/benchmark.log") 2>/dev/null"; then
...
_fetch_artifacts() {
  ...
  for f in benchmark.log scale_timing.env scale.log metrics_timeseries.csv scale_events.csv; do
    _scp_from "${remote_base}/${f}" "${local_run_dir}/${f}" 2>/dev/null \
      || echo "WARN: ${f} not found"
  done
  _scp_from "${remote_base}/benchmark.conf" "${local_run_dir}/benchmark.conf" 2>/dev/null || true
  ...
  if [[ -n "${report_rel}" ]]; then
    _scp_from "${remote_base}/${report_rel}" "${local_run_dir}/$(basename "${report_rel}")" 2>/dev/null \
      || echo "WARN: report not found at ${report_rel}"
  else
    _scp_from "${remote_base}/scaling_analysis_report.html" "${local_run_dir}/scaling_analysis_report.html" 2>/dev/null \
      || _scp_from "${remote_base}/scaling_report.html" "${local_run_dir}/scaling_report.html" 2>/dev/null \
      || echo "WARN: no HTML report yet (generate on droplet with scripts/generate_scaling_report.sh)"
  fi

  {
    ...
    echo "droplet=${DROPLET_USER}@${DROPLET_HOST}"
    echo "remote_repo=${REMOTE_REPO}"
    [[ -f "${local_run_dir}/scaling_analysis_report.html" ]] && echo "report_html=${local_run_dir}/scaling_analysis_report.html"
    [[ -f "${local_run_dir}/scaling_report.html" ]] && echo "report_html=${local_run_dir}/scaling_report.html"
    [[ -f "${local_run_dir}/metrics_timeseries.csv" ]] && echo "timeseries_csv=${local_run_dir}/metrics_timeseries.csv"
...
main() {
  ...
  if [[ ! -f "${ARTIFACTS_DIR}/${RESULTS_DIR##*/}/benchmark.log" ]]; then
    echo "ERROR: benchmark.log was not fetched" >&2
    exit 1
  fi

  echo ""
  echo "=== CI scaling benchmark complete ==="
