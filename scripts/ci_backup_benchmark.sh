#!/usr/bin/env bash
# Orchestrate a backup benchmark on a remote droplet (GitHub Actions scheduler or local).
#
...
DROPLET_SSH_PORT=22
REMOTE_REPO=""
REMOTE_BENCHMARK_CONF="backup-benchmarking/benchmark.conf"
CI_GIT_SYNC=1
DROPLET_GIT_BRANCH=""
...
_dump_failure_diagnostics() {
  ...
  ctl_q="$(printf '%q' "${REMOTE_REPO}/scripts/backup_run_ctl.sh")"

  {
    ...
    _ssh "tail -n 100 ${repo_q}/backup-benchmarking/results/control_wrapper.log" 2>&1 || true
...
_preflight_remote() {
  ...
  conf_q="$(printf '%q' "$(_remote_conf_path)")"
  ctl_q="$(printf '%q' "${REMOTE_REPO}/scripts/backup_run_ctl.sh")"
  run_q="$(printf '%q' "${REMOTE_REPO}/backup-benchmarking/run_benchmark.sh")"
...
_ctl_status() {
  ...
  conf_q="$(printf '%q' "$(_remote_conf_path)")"
  ctl_q="$(printf '%q' "${REMOTE_REPO}/scripts/backup_run_ctl.sh")"
  _ssh_capture "backup_run_ctl status" \
...
_start_run() {
  ...
  _step "Starting backup benchmark on droplet"
  start_out="$(_ssh_capture "backup_run_ctl start" "set -euo pipefail; cd ${repo_q}; BENCHMARK_CONF=${conf_q} ${ctl_q} start")"
  echo "${start_out}"
  ...
  if [[ -z "${results_dir}" || "${results_dir}" == "backup-benchmarking/results/run_pending" ]]; then
...
_fetch_artifacts() {
  ...
  for f in benchmark.log run_timing.env backup_profiler.log benchmark_with_backup_status.csv backup_events.csv; do
  ...
  if [[ -n "${report_rel}" ]]; then
    ...
    _scp_from "${remote_base}/backup_benchmark_report.html" "${local_run_dir}/backup_benchmark_report.html" 2>/dev/null \
      || echo "WARN: no HTML report yet (generate on droplet with backup-benchmarking/scripts/generate_report.py)"
  fi

  {
    ...
    echo "droplet=${DROPLET_USER}@${DROPLET_HOST}"
    echo "remote_repo=${REMOTE_REPO}"
    [[ -f "${local_run_dir}/backup_benchmark_report.html" ]] && echo "report_html=${local_run_dir}/backup_benchmark_report.html"
    [[ -f "${local_run_dir}/benchmark_with_backup_status.csv" ]] && echo "csv=${local_run_dir}/benchmark_with_backup_status.csv"
...
main() {
  ...
  echo ""
  echo "=== CI backup benchmark complete ==="
