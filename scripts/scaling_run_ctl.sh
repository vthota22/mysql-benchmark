#!/usr/bin/env bash
# Droplet-side helper for scaling benchmarks: start/status/log/list.
# Mirrors scripts/failover_run_ctl.sh for scaling-benchmarking/run_benchmark.sh.
set -euo pipefail
...
_pick_report_path() {
  ...
  local candidate
  for candidate in \
    "${base}/scaling_analysis_report.html" \
    "${base}/scaling_report.html"
  do
    if [[ -f "${candidate}" ]]; then
...
_cmd_start() {
  ...
  if _pid_alive "${existing}"; then
    echo "ERROR: scaling benchmark already running (pid ${existing})" >&2
    exit 1
  fi
  rm -f "${LOCK_FILE}"
  ...
  if [[ -z "${results_dir}" ]]; then
    results_dir="scaling-benchmarking/results/run_pending"
  fi
  ...
  if ! _pid_alive "${pid}"; then
    ...
    if [[ -f "${FEATURE_DIR}/results/control_wrapper.log" ]]; then
      echo "--- scaling-benchmarking/results/control_wrapper.log (last 40 lines) ---" >&2
...
_cmd_log() {
  ...
  if [[ -z "${results_dir}" ]]; then
    echo "No scaling run directory found." >&2
    exit 1
  fi
...
_cmd_list() {
  ...
  cd "${REPO_ROOT}"
  ls -1dt scaling-benchmarking/results/run_* 2>/dev/null | head -n "${limit}" | while IFS= read -r d; do
    [[ -n "${d}" ]] || continue
    printf 'RUN|%s\n' "${d}"
    local completed=0
